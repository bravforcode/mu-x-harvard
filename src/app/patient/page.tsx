...
export default function PatientHome() {
  ...
  // Pretend logged in as first patient
  ...
  const isRegistrationStep =
    !!activeStep &&
    (activeStep.department === "Registration" ||
    ...
    <div className="flex flex-col gap-5 animate-fade-in">
      ...
      <div className="bg-card border border-border rounded-2xl p-5 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-slate-400 text-xs font-medium">{currentPatient.hn} &middot; Today</p>
        </div>
      ...
      <div className="bg-primary rounded-2xl p-6 text-white relative overflow-hidden">
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-between bg-white/20 rounded-xl p-3">
            ...
            <span className="text-2xl font-bold font-mono">{completedCount}<span className="text-white/50 text-lg">/{totalSteps}</span></span>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-start justify-between mb-4 pt-1">
            <div>
              ...
              <h2 className="text-lg font-bold text-slate-800">
                {isRegistrationStep ? "Check-in at Registration" : activeStep.title}
              </h2>
              <p className="text-slate-500 text-sm">{activeStep.location} &middot; {activeStep.floor}</p>
            </div>
            {activeStep.queueNumber && (
              <StatusBadge variant="active" pulse>{activeStep.queueNumber}</StatusBadge>
            )}
          </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/patient/timeline" className="text-xs text-primary font-semibold hover:underline">View details</Link>
        </div>
            ...
            return (
              <div key={step.id} className="relative">
                ...
                <div className={cn(isActive && "bg-accent -mt-1 -ml-1 p-3 rounded-xl border border-primary/20", !isActive && "py-1")}>
                  ...
                  <p className={cn(
                    "text-xs mt-0.5",
                    isActive ? "text-primary/60 font-medium" : "text-slate-400",
                  )}>
